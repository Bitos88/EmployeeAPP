...
//  EmployeeApp
//
//  Created by Alberto Alegre Bravo on 6/6/23.
//
...
final class AddEmployeeViewModel: ObservableObject {
    ...
    init() {
        Task { @MainActor in
            departamentos = try await persistance.getDepartments()
        }
    }
    
    func postEmployee() {
        let emailRegex = try? Regex(#"(?:[a-z0-9!#$%&'+/=?^_`{|}~-]+(?:.[a-z0-9!#$%&'+/=?^_`{|}~-]+)|"(?:[\x01-\x08\x0b\x0c\x0e-\x1f\x21\x23-\x5b\x5d-\x7f]|\[\x01-\x09\x0b\x0c\x0e-\x7f])")@(?:(?:[a-z0-9](?:[a-z0-9-][a-z0-9])?.)+[a-z0-9](?:[a-z0-9-][a-z0-9])?|[(?:(?:(2(5[0-5]|[0-4][0-9])|1[0-9][0-9]|[1-9]?[0-9])).){3}(?:(2(5[0-5]|[0-4][0-9])|1[0-9][0-9]|[1-9]?[0-9])|[a-z0-9-]*[a-z0-9]:(?:[\x01-\x08\x0b\x0c\x0e-\x1f\x21-\x5a\x53-\x7f]|\[\x01-\x09\x0b\x0c\x0e-\x7f])+)])"#)
                
        if firstName.isEmpty {
            wrongFieldMessage = "First name cannot be empty"
        }
        
        if lastName.isEmpty {
            wrongFieldMessage = "Last name cannot be empty"
        }
        ...
        if email.isEmpty || !((try? emailRegex?.wholeMatch(in: email)) == nil) {
            wrongFieldMessage = "Email cannot be empty or it has bad format"
        }
        ...
        if wrongFieldMessage.isEmpty {
            let newEmployee = NewEmployee(username: userName,
                                           ...
                                           email: email,
                                           address: address,
                                           avatar: "https://robohash.org/enimsolutaperferendis.png",
                                           zipcode: zipcode,
            ...
            Task {
                await persistance.postEmployee(employee: newEmployee)
            }
